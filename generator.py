...
def generate_sbh_instance(
    ...
    """
    # Wygeneruj losową sekwencję DNA długości n
    dna = ''.join(random.choices('ATCG', k=n))
    ideal_kmers = [dna[i : i + k] for i in range(n - k + 1)]
    ideal_set = set(ideal_kmers)
    # Skopiuj idealne k-mery i usuń num_neg_errors (błędy negatywne)
    ...
    # Dodaj num_pos_errors k-merów fałszywych (błędy pozytywne)
