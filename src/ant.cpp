...
void two_opt_on_trail(Ant& ant,
    ...
    if (best_len < ant.length) {
        ...
        ant.length = best_len;
        if (n >= 0) {
            ant.seq_len = std::max(ant.seq_len, static_cast<size_t>(n));
        }
    }
}
...
void update_pheromones(std::vector<std::vector<double>>& pheromone,
    ...
    size_t num_kmers = pheromone.size();
    ...
    for (const Ant& ant : ants) {
        if (ant.seq_len < n || ant.length <= 0) continue;
...
void update_ants(const std::vector<Ant>& ants,
    ...
    const size_t max_steps = ants[0].trail.size();
    ...
    for (Ant ant : ants) {
        ...
        while (ant.seq_len < n && ant.trail_len < max_steps) {
            ...
            if (ideal) {
                // Krok 1: Overlap k-1
                std::vector<std::pair<size_t, int>> fresh_candidates;
                std::vector<std::pair<size_t, int>> reuse_candidates;
                ...
                bool found = false;
                for (int o = k - 2; o >= 1; --o) {
                    std::vector<std::pair<size_t, int>> new_fresh_candidates;
                    std::vector<std::pair<size_t, int>> new_reuse_candidates;
                ...
                if (!Y.empty()) {
                    ...
                    if (!path.empty() && path.size() >= 2) {
                        for (size_t idx = 1; idx < path.size(); ++idx) {
                            ...
                            int w_uv = weight_opt.has_value() ? *weight_opt : static_cast<int>(k);
            ...
            if (only_neg || only_pos) {
                // Krok 1: ACO
                std::vector<std::tuple<size_t, int, int>> candidates;
                std::vector<double> probs;
                for (const auto& edge : adj_matrix[u]) {
                    size_t j = edge.target;
                    if (ant.used_count[j] < repeat_limits[j]) {
                        double pher = pheromone[u][j] > 0.0 ? pheromone[u][j] : 1e-12;
                        double heur = static_cast<double>(edge.overlap);
...
std::string run_aco(const std::vector<std::string>& kmers,
    ...
    std::vector<Ant> ants;
    size_t max_steps = kmers.size();
    for (int i = 0; i < num_ants; ++i) {
        Ant ant(max_steps, kmers.size());
        if (found_start) {
            ant.trail.emplace_back(start_idx);
            ant.used_count[start_idx]++;
        }
    ...
    while (true) {
        ...
        for (auto& ant : ants) {
            ...
            std::vector<size_t> trail_indices(ant.trail.begin(), ant.trail.end());
