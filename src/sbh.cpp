...
AdjacencyList build_adjacency_matrix(const std::vector<std::string>& kmers, int k) {
    for (const auto& kmer : kmers) {
        if (kmer.length() == static_cast<size_t>(k)) continue;
    ...
    std::unordered_map<int, std::unordered_map<std::string, std::vector<size_t>>> prefix_map;
    ...
    for (size_t i = 0; i < num_kmers; i++) {
        ...
        for (int o = k - 1; o >= 1; o--) {
            if (ui.length() >= static_cast<size_t>(o)) {
                ...
                if (prefix_map_for_overlap != prefix_map.end()) {
                    ...
                    if (suffix_entry != prefix_map_for_overlap->second.end()) {
                        for (size_t j : suffix_entry->second) {
                            if (j != i) {
                                int weight = k - o;
...
std::vector<size_t> find_eulerian_path(const AdjacencyList& adj_matrix) {
    ...
    while (!circuit.empty()) {
        if (!temp_adj_matrix[current].empty()) {
            ...
            size_t next = temp_adj_matrix[current].back().target;
...
bool find_hamiltonian_path_helper(const AdjacencyList &adj_matrix, std::vector<size_t> &path,
                                  std::vector<bool> &visited, size_t current, size_t target_length) {

    if (path.size() == target_length) return true;

    for (const auto& edge : adj_matrix[current]) {
        size_t next = edge.target;
        if (!visited[next]) {
            visited[next] = true;
            path.push_back(next);
...
std::string sequencing_by_hybridization(const std::vector<std::string>& kmers,
    ...
    std::vector<size_t> path = find_eulerian_path(adj_matrix);
    if (path.empty()) {
...
std::string sequencing_by_hybridization_with_start(const std::vector<std::string>& kmers,
    ...
    std::vector<size_t> path = find_path_from_start(adj_matrix, start_idx, kmers.size());
...
std::vector<std::string> generate_kmers(const std::string& sequence, int k) {
    ...
    if (sequence.length() < static_cast<size_t>(k)) return kmers;
...
std::pair<size_t, std::vector<size_t>> dijkstra_shortest_path(const AdjacencyList& adj_matrix,
    ...
    size_t num_kmers = adj_matrix.size();
    constexpr size_t NO_PREV = std::numeric_limits<size_t>::max();
    constexpr int INF = std::numeric_limits<int>::max();

    std::vector dist(num_kmers, INF);
    std::vector prev(num_kmers, NO_PREV);
    dist[start] = 0;
    ...
    if (best_target == NO_PREV) return {NO_PREV, {}};

    std::vector<size_t> path;
    for (size_t node = best_target; node != NO_PREV; node = prev[node]) {
        path.push_back(node);
    }
    std::ranges::reverse(path);
